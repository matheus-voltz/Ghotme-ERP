...
<div class="card">
  <div class="card-header border-bottom d-flex justify-content-between align-items-center">
    <h5 class="card-title mb-0">Gestão de {{ niche('entities') }}</h5>
    <button class="btn btn-primary add-new" data-bs-toggle="offcanvas" data-bs-target="#offcanvasAddVehicles">
      <i class="ti tabler-plus me-1"></i> Novo {{ niche('entity') }}
    </button>
  </div>
  ...
  <div class="offcanvas offcanvas-end" style="width: 500px !important;" tabindex="-1" id="offcanvasAddVehicles" aria-labelledby="offcanvasAddVehiclesLabel">
    <div class="offcanvas-header border-bottom">
      <h5 id="offcanvasAddVehiclesLabel" class="offcanvas-title">Cadastrar {{ niche('entity') }}</h5>
    ...
    <div class="offcanvas-body mx-0 flex-grow-0 p-6 h-100">
      <form class="add-new-vehicles pt-0" id="addNewVehicleForm">
        ...
        <div class="mb-6">
          <label class="form-label">{{ niche('identifier') }}</label>
        ...
        <div class="row">
          <div class="col-md-12 mb-6">
            <label class="form-label">{{ niche('year') }}</label>
            <input type="number" name="ano_fabricacao" id="add-vehicle-ano-fabricacao" class="form-control" placeholder="2022" />
          </div>
        </div>

        <div class="mb-6">
          <label class="form-label">{{ niche('secondary_identifier') }}</label>
          <input type="text" name="renavam" id="add-vehicle-renavam" class="form-control" placeholder="Opcional" />
        </div>
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg">
    ...
    <div class="modal-content">
      ...
      <div class="modal-header">
        ...
        <h5 class="modal-title">Dossiê do {{ niche('entity') }}</h5>
