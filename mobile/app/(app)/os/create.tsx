    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            Alert.alert("Sucesso", "Ordem de Serviço criada com sucesso!", [
                ...
                { 
                    text: "WhatsApp e Sair 💬", 
                    onPress: () => {
                        sendWhatsApp();
                        router.back();
                    } 
                },
                { text: "Apenas Sair", onPress: () => router.back(), style: 'cancel' }
            ]);
        } catch (error: any) {
...
const styles = StyleSheet.create({
    ...
    statusChip: { flex: 1, paddingVertical: 12, borderRadius: 12, borderWidth: 1, alignItems: 'center', justifyContent: 'center' },
    statusText: { fontWeight: '600', fontSize: 14 },
    textAreaContainer: { borderWidth: 1, borderRadius: 12, padding: 15, height: 120 },
    textArea: { flex: 1, fontSize: 16, textAlignVertical: 'top' },
    infoBox: { flexDirection: 'row', alignItems: 'flex-start', padding: 16, borderRadius: 12, marginBottom: 30 },
    infoText: { flex: 1, fontSize: 13, marginLeft: 10, lineHeight: 20 },
    footer: { padding: 20, paddingBottom: Platform.OS === 'ios' ? 40 : 20, borderTopWidth: 1, borderTopColor: '#f0f0f0' },
